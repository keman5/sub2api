...
func (h *GatewayHandler) handleResponsesFailoverExhausted(c *gin.Context, lastErr *service.UpstreamFailoverError, streamStarted bool) {
	...
	if lastErr != nil && lastErr.IsCredentialFailure() {
	...
	} else if lastErr != nil && service.IsOpenAISilentRefusalErrorBody(responseBody) {
		...
		status, code, message = http.StatusTooManyRequests, "rate_limit_error", service.UpstreamFailureClientMessage(
			statusCode,
			responseBody,
	...
	if code == "server_error" && lastErr != nil && !lastErr.IsCredentialFailure() && !lastErr.IsOpenAICapacityShed() {
		status, code, message = h.mapUpstreamError(statusCode)
